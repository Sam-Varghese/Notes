...
- In order to get only the element (without its tag), then we call `innerHTML`, as follows:
...
- The document object represents your web page.
- If you wanna access any element in an HTML page, you always start with accessing the document object.
...
- Instead of finding elements of a particular tag in document, we can also find them in an a container of a particular id. Ex:
...
- In order to access an element in JavaScript by its class name, we use `document.getElementsByClassName("Class")`.
...
- In order to acces all elements ( suppose `p`), of a given class ( suppose `Class`), we use following CSS selector query:

<hr>

`var x= document.querySelectorAll("p.Class")`
